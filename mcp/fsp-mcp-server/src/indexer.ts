...
import type { DocumentSection, IndexerOptions } from './types.js';
...
class FSPIndexer {
  ...
  private extractTextContent(elem: cheerio.Cheerio<any> | null): string {
  ...
  private extractCodeBlocks($: cheerio.CheerioAPI): Array<{ code: string; language: string }> {
  ...
  private detectCodeLanguage(code: string): string {
    ...
    if (code.includes('typedef') && code.includes('struct')) return 'c';
    return 'unknown';
  }

  private determineSectionType(title: string, content: string, filename: string): DocumentSection['sectionType'] {
    const titleLower = title.toLowerCase();
    const filenameLower = filename.toLowerCase();

    if (titleLower.includes('module') || filenameLower.startsWith('group_')) return 'module';
    if (titleLower.includes('function') || titleLower.includes('api') || titleLower.includes('reference')) return 'api';
    if (titleLower.includes('struct') || titleLower.includes('typedef') || titleLower.includes('union') || titleLower.includes('enum')) return 'struct';
    if (titleLower.includes('example') || titleLower.includes('sample') || titleLower.includes('usage')) return 'example';

    return 'general';
  }

  private extractModuleName(filename: string, pageTitle: string): string {
  ...
  private parseModulePage(filePath: string): DocumentSection[] {
  ...
  private async getAllHtmlFiles(): Promise<string[]> {
  ...
  async indexDocumentation(forceReindex = false): Promise<number> {
